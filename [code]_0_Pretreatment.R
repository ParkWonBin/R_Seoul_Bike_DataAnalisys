...
x = read.csv(paste0(p,f[1]), fileEncoding = 'utf-8')
for (i in 2:length(f)){
  y = read.csv(paste0(p,f[i]), fileEncoding = 'utf-8')
  names(y) = names(x)
  x = rbind(x,y)
  print(paste0(f[i]))
}
...
# 전체 정수형으로 형변환
y = x
y$bike = as.integer(x$bike)
y$date = as.integer(x$date)
y$start = as.integer(x$start) # 강제 형변환으로 NA 발생
y$end = as.integer(x$end) # 강제 형변환으로 인한 NA 발생
y$time =as.integer(x$time)
y$dist = as.integer(x$dist)

x[is.na(y$start),]



write.csv(x, file='Bike-Use.csv', row.names = FALSE)
