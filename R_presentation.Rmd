...
### 데이터 전처리
서울시 공공데이터 자료실에 게시된 raw-data에 사소한 문제가 있었다. 2020.1월 ~ 2020.5월 파일에 한글 인코딩은 중간중간 깨져 데이터 열 구분이 틀어져있는 경우가 종종 있다. 그리고 대여소 ID를 기록하는 열은 정수형 데이터가 예상되지만 가끔씩 대여 소명 (와트컴, 중량센터, 상암센터 등)이 들어가 있기도 하다. 


 - [깃허브 주소](https://github.com/ParkWonBin/R_Seoul_Bike_DataAnalisys)
...
데이터가 계절성을 갖고 있기 때문에, 계절성에 대한 직관을 잃지 않고자 월별로 data의 색깔을 다르게 칠했다.
